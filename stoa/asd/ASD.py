...
from stoa.asd.data.dataset import PoisonLabelDataset, MixMatchDataset
from stoa.asd.data.utils import (
    get_dataset,
    get_loader,
    get_transform,
)
from stoa.asd.model.model import LinearModel
from stoa.asd.model.utils import (
    get_criterion,
    get_network,
    get_optimizer,
    get_scheduler,
...
from stoa.asd.utils.setup import (
    get_logger,
    get_saved_dir,
    get_storage_dir,
    load_config,
...
from stoa.asd.utils.trainer.log import result2csv
from stoa.asd.utils.trainer.semi import mixmatch_train, linear_test, poison_linear_record

def main():
    print("===Setup running===")
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="./config/imagenette_pr_0.003_seed_num_10.yaml")
...
def main_worker(args, config):
    ...
    for epoch in tqdm(range(0, config["num_epochs"])):
        ...
        if epoch < config["global"]["epoch_first"]:
            ...
            split_idx, auc = class_aware_loss_guided_split(record_list, indice, all_data_info, choice_num, logger, poisoned_train_data.poison_idx)
            ...
            split_idx, auc = class_agnostic_loss_guided_split(record_list, config["global"]["epsilon"], logger, poisoned_train_data.poison_idx)
            ...
            if "Adam" in meta_optimizer_config:
                meta_optimizer = torch.optim.Adam(meta_virtual_model.parameters(), **meta_optimizer_config["Adam"])
                # lr, weight_decay = 0.1, 1e-4
                # meta_optimizer = torch.optim.Adam(meta_virtual_model.parameters(),
                #                             lr=lr,
                #                             weight_decay=weight_decay)
            elif "SGD" in meta_optimizer_config:
                # meta_optimizer = torch.optim.SGD(meta_virtual_model.parameters(), **meta_optimizer_config["SGD"])
                lr, momentum, weight_decay = 0.1, 0.9, 1e-4
                meta_optimizer = torch.optim.SGD(meta_virtual_model.parameters(),
                                            lr=lr,
                                            momentum=momentum,
                                            weight_decay=weight_decay,
                                            nesterov=True)
            ...
            split_idx, auc = meta_split(record_list, meta_record_list, config["global"]["epsilon"], logger, poisoned_train_data.poison_idx)
        ...
        split_results['auc'] = auc
        xloader = get_loader(
            xdata, config["semi"]["loader"], shuffle=True, drop_last=False
        )
        uloader = get_loader(
        ...
        poison_test_result = linear_test(
            linear_model, poison_test_loader, criterion, logger, target_class=poisoned_train_data.target_label
        )
...
def class_aware_loss_guided_split(record_list, has_indice, all_data_info, choice_num, logger, gt_poison_index):
    ...
    return clean_pool_idx, auc


def class_agnostic_loss_guided_split(record_list, ratio, logger, gt_poison_index):
