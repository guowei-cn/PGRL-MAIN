...
from lib.dataLoader import calculate_tpr_fpr_no_indices
...
def get_semi_idx(record_list, ratio):
    ...
    bn_indice = loss.argsort()[: int(len(loss) * ratio)]
    semi_idx[bn_indice] = 1
    suspect_indice = loss.argsort()[-int(len(loss) * ratio):]
    # TODO: based on the half threshold
    gt_poison_l = poison
    pd_poison_l = np.array([0 if element == 1 else 1 for element in semi_idx])
    TPR, FPR, ACC = calculate_tpr_fpr_no_indices(gt_poison_l, pd_poison_l)
    print(TPR, FPR, ACC)
    auc = roc_auc_score(gt_poison_l, loss)
    return semi_idx, bn_indice, suspect_indice, loss, TPR, FPR, auc
