...
from stoa.dbd.utils.trainer.log import AverageMeter, Record, tabulate_step_meter, tabulate_epoch_meter


def simclr_train(model, loader, criterion, optimizer, device, debugging_flag):
...
def linear_test(model, loader, criterion, args):
    loss_meter = AverageMeter("loss")
    acc_meter = AverageMeter("acc")
    meter_list = [loss_meter, acc_meter]
...
def poison_linear_train(model, loader, criterion, optimizer, args, frozen=True):
    loss_meter = AverageMeter("loss")
...
def poison_linear_record(model, loader, criterion, args):
