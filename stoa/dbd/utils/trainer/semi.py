...
def mixmatch_train(
    model, xloader, uloader, criterion, optimizer, epoch, args, **kwargs,
):
    ...
    model.train()
    for batch_idx in tqdm(range(kwargs["train_iteration"])):
        try:
            xbatch = next(xiter)
        ...
        xinput = xinput.to(args.device)
        xtarget = xtarget.to(args.device)
        uinput1 = uinput1.to(args.device)
        uinput2 = uinput2.to(args.device)
        if len(xinput.shape) == 3:
            xinput = xinput.unsqueeze(1)
            uinput1 = uinput1.unsqueeze(1)
            uinput2 = uinput2.unsqueeze(1)
