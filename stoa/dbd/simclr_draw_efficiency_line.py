...
def main_worker(device, args, config):
    ...
    memory_loader = DataLoader(clean_tra_data, batch_size=128, shuffle=False)

    clean_test_loader = DataLoader(clean_test_data, batch_size=128, shuffle=False)
    poison_test_loader = DataLoader(poison_test_data, batch_size=128, shuffle=False)
    ...
    knn_acc, back_acc = knn_monitor_fre(self_model,
                                        memory_loader, clean_test_loader, 0, args.device,
                                        classes=args.num_class,
                                        subset=False,
                                        k=100,
                                        backdoor_loader=poison_test_loader,
                                        )
    epoch_acc_asr = [[0, knn_acc, back_acc]]
    for epoch in tqdm(range(1, epoch_num + 1)):
        ...
        if epoch % 5 == 0:
            ...
            epoch_acc_asr.append([epoch, knn_acc, back_acc])

    np.save('simclr_efficiency.npy', np.array(epoch_acc_asr))
