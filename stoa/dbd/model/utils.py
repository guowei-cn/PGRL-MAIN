...
def get_network(network_config,  poison_type='adaptivecifar10'):
    if "resnet18_cifar" in network_config:
        if poison_type == 'ultrasonic':
            network_config["resnet18_cifar"]['in_channel'] = 1
...
def get_optimizer(model, optimizer_config):
    if "Adam" in optimizer_config:
        ...
        # params = [
        #     {'params': model.backbone.parameters(), 'lr': optimizer_config["Adam"]['lr'] * 0.5},
        #     {'params': model.linear.parameters(), 'lr': optimizer_config["Adam"]['lr']}
        # ]
        # optimizer = torch.optim.Adam(params, betas=(0.99, 0.999))
...
def load_state(
    model, resume, ckpt_dir, args, optimizer=None, scheduler=None, is_best=False
):
    ...
    if resume == "False":
        ...
        # Load checkpoint.
        if resume == "":
            ckpt_path = os.path.join(ckpt_dir, "epoch100.pt")
        else:
