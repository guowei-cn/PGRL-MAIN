...
# os.environ["CUDA_VISIBLE_DEVICES"] = "%s" % args.device
...
if args.dataset != 'ember' and args.dataset != 'imagenet' and args.dataset != 'ultrasonic' and args.dataset != 'freq'\
        and args.dataset != 'freq_meg_500' and args.dataset != 'pattern' and args.dataset != 'wanet':
    ...
    poison_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/poisonTrain_pr_{}_t_down'.\
        format(args.poison_rate)

    poisoned_set = tools.AUDIO_Dataset(data_dir=poison_set_dir, transforms=True)
    cleansed_set_indices_dir = supervisor.get_cleansed_set_indices_dir(args)
    print('load : %s' % cleansed_set_indices_dir)
    cleansed_set_indices = torch.load(cleansed_set_indices_dir)

    poison_files = np.load(os.path.join(poison_set_dir, 'poison_file.npy'))
    poisoned_indices = []
    for index, f in enumerate(poisoned_set.files):
        f = f.split(args.dataset)[1][1:]
        if f in poison_files:
            poisoned_indices.append(index)

elif 'freq' in args.dataset or args.dataset =='pattern' or args.dataset == 'wanet':

    target_class = 0 if 'freq' in args.dataset else 2
    poison_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/poisonTrain_pr_{}_t_{}'.format(
        args.dataset, args.poison_rate, target_class)

    print('inspected dataset : %s' % poison_set_dir)

    poisoned_set = tools.IMAGE_Dataset(data_dir=poison_set_dir, transforms=data_transform_aug)
    cleansed_set_indices_dir = supervisor.get_cleansed_set_indices_dir(args)
    print('load : %s' % cleansed_set_indices_dir)
    cleansed_set_indices = torch.load(cleansed_set_indices_dir)
    ...
    poison_files = np.load(os.path.join(poison_set_dir, 'poison_file.npy'))
    poisoned_indices = []
    for index, f in enumerate(poisoned_set.data):
        f = f.split(args.dataset)[1][1:]
        if f in poison_files:
            poisoned_indices.append(index)
...
if args.dataset != 'ember' and args.dataset != 'imagenet' and args.dataset != 'ultrasonic' and args.dataset != 'freq'\
    ...
    poison_transform = supervisor.get_poison_transform(poison_type=args.poison_type, dataset_name=args.dataset,
                                                       target_class=config.target_class_origin[args.dataset], trigger_transform=trigger_transform,
    ...
    target_name = 'down'
    target_class_label = test_set.classes_name.index(target_name)
    poison_test_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/poisonTest'
    poison_test_set = tools.AUDIO_Dataset(data_dir=poison_test_set_dir, fixed_label=target_class_label)
    ...
    test_set = tools.IMAGE_Dataset(data_dir=test_set_dir, transforms=data_transform_no_aug)
    ...
    target_name = 'airplane' if 'freq' in args.dataset else 'bird'
    target_class_label = test_set.classes_name.index(target_name)
    poison_test_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/poisonTest'.format(args.dataset)
    poison_test_set = tools.IMAGE_Dataset(data_dir=poison_test_set_dir, transforms=data_transform_no_aug, fixed_label=target_class_label)
...
if args.dataset != 'ember':
    ...
    if os.path.exists(supervisor.get_model_dir(args, cleanse=True)):  # exit if there is an already trained model
        ...
        tools.test(model=model, test_loader=test_set_loader, poison_test=False,
                   ...
                   source_classes=None, args=args)
        tools.test(model=model, test_loader=poison_test_set_loader, poison_test=False,
                   num_classes=num_classes, source_classes=None, args=args)
        exit(0)
...
for epoch in range(1,epochs+1):
    ...
    if args.dataset != 'ember':
        if epoch % 20 == 0:
            ...
            elif args.dataset == 'ultrasonic' or 'freq' in args.dataset or args.dataset == 'pattern' or args.dataset == 'wanet':
                tools.test(model=model, test_loader=test_set_loader, poison_test=False,
                           num_classes=num_classes,
                           source_classes=None, args=args)
                tools.test(model=model, test_loader=poison_test_set_loader, poison_test=False,
