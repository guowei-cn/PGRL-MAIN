...
# extract features
def get_features(data_loader, model, args):
    '''
    ...
    with torch.no_grad():
        ...
        for i, (ins_data, ins_target) in enumerate(tqdm(data_loader)):
            ...
            ins_data, ins_target = ins_data.to(args.device), ins_target.to(args.device)
...
def identify_poison_samples_simplified(inspection_set, clean_indices, model, num_classes, args):
    '''
    ...
    gt_confidence_inspection, loss_vals = get_features(inspection_split_loader, model, args)
    ...
    for target_class in range(num_classes):
        ...
        temp_feats = torch.FloatTensor(
            feats_inspection[class_indices[target_class]]).to(args.device)
...
def pretrain(args, debug_packet, arch, num_classes, weight_decay, pretrain_epochs, distilled_set_loader, criterion,
    ...
    for epoch in range(1, pretrain_epochs + 1):  # pretrain backdoored base model with the distilled set
        ...
        for batch_idx, (data, target) in enumerate(tqdm(distilled_set_loader)):
            optimizer.zero_grad()
            data, target = data.to(args.device), target.to(args.device)  # train set batch
            output = model(data)
            ...
            if args.debug_info:
                ...
                if dataset_name == 'cifar10':
                    tools.test(model=model, test_loader=debug_packet['test_set_loader'], poison_test=True,
                           ...
                           source_classes=debug_packet['source_classes'], args=args)
                elif dataset_name == 'ultrasonic' or 'freq' in dataset_name or dataset_name == 'pattern' or dataset_name == 'wanet':
                    tools.test(model=model, test_loader=debug_packet['test_set_loader'], poison_test=False, num_classes=num_classes,
                               source_classes=debug_packet['source_classes'], args=args)
                    tools.test(model=model, test_loader=debug_packet['poison_test_set_loader'], poison_test=False,
                               num_classes=num_classes, source_classes=debug_packet['source_classes'], args=args)
                elif dataset_name == 'imagenet':
                    tools.test_imagenet(model=model, test_loader=debug_packet['test_set_loader'],
                                        test_backdoor_loader=debug_packet['test_set_backdoor_loader'], args=args)
                elif dataset_name == 'ember':
                    tools.test_ember(model=model, test_loader=debug_packet['test_set_loader'],
                                     backdoor_test_loader=debug_packet['backdoor_test_set_loader'], args=args)
                else:
                    print('no implementation of {}'.format(dataset_name))
                    exit(-1)

    base_ckpt = model.state_dict()
...
def confusion_train(args, params, inspection_set, debug_packet, distilled_set_loader, clean_set_loader, confusion_iter, arch,
    ...
    for batch_idx in tqdm(range(distillation_iters)):
        ...
        try:
            ...
            clean_set_iters = iter(clean_set_loader)
            data_shift, target_shift = next(clean_set_iters)
        data_shift, target_shift = data_shift.to(args.device), target_shift.to(args.device)
        ...
        if (batch_idx + 1) % 500 == 0:
            ...
            if args.debug_info:
                ...
                if dataset_name != 'ember' and dataset_name != 'imagenet' and dataset_name != 'ultrasonic' and \
                        dataset_name != 'freq' and dataset_name != 'freq_meg_500' and dataset_name != 'wanet' \
                        and dataset_name != 'pattern':
                    tools.test(model=model, test_loader=debug_packet['test_set_loader'], poison_test=True,
                           poison_transform=debug_packet['poison_transform'], num_classes=num_classes,
                           source_classes=debug_packet['source_classes'], args=args)
                elif dataset_name == 'ultrasonic' or 'freq' in dataset_name or dataset_name == 'wanet' or dataset_name == 'pattern':
                    tools.test(model=model, test_loader=debug_packet['test_set_loader'], poison_test=False, num_classes=num_classes,
                               source_classes=debug_packet['source_classes'], args=args)
                    tools.test(model=model, test_loader=debug_packet['poison_test_set_loader'], poison_test=False,
                               num_classes=num_classes, source_classes=debug_packet['source_classes'], args=args)
...
def distill(args, params, inspection_set, n_iter, criterion_no_reduction, debug_packet=None,
    ...
    if args.debug_info:
        ...
        poison_indices = debug_packet['poison_indices'] # torch.load(os.path.join(inspection_set_dir, 'poison_indices'))
        # calculate the AUC
        auc = roc_auc_score(np.array([1 if i in poison_indices else 0 for i in range(len(loss_array))]), -loss_array)
