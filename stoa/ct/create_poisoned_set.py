...
if args.poison_type == 'dynamic':
    ...
    if args.dataset == 'gtsrb':
        ...
        # check existence of os.path.join(data_dir, 'imagenette') if yes, skip following one line code
        if not os.path.exists(os.path.join(data_dir, 'imagenette')):
...
if args.poison_type in ['badnet', 'blend', 'clean_label',
    ...
    elif args.poison_type == 'trigger_10':
        from poison_tool_box import trigger_10
        poison_generator = trigger_10.poison_generator(img_size=img_size, dataset=train_set,
                                                       poison_rate=args.poison_rate, trigger_mark=trigger, trigger_mask=trigger_mask,
                                                       path=poison_set_img_dir, target_class=config.target_class[args.dataset])
