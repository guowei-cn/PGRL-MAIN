...
parent_path = os.path.join(os.getcwd().split('PGRL-main')[0], 'PGRL-main')
...
if args.dataset == 'cifar10' or 'freq' in args.dataset or args.dataset == 'pattern' or args.dataset == 'wanet':
...
elif args.dataset == 'ember' or args.dataset == 'ultrasonic':
...
if args.dataset == 'cifar10' or args.dataset == 'imagenette':
    ...
    milestones = torch.tensor([])
    batch_size = 128
elif 'freq' in args.dataset or args.dataset == 'wanet' or args.dataset == 'pattern':
...
if args.dataset != 'ember' and args.dataset != 'imagenet' and args.dataset != 'ultrasonic' and args.dataset != 'freq' and args.dataset != 'freq_meg_500' and args.dataset != 'pattern' and args.dataset != 'wanet':
    ...
    poisoned_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/poisonTrain_pr_{}_t_down'.format(args.poison_rate)
    poison_indices_path = os.path.join(poisoned_set_img_dir, 'poison_file.npy')


    print('dataset : %s' % poisoned_set_img_dir)

    poisoned_set = tools.AUDIO_Dataset(data_dir=poisoned_set_img_dir, transforms=True)

    poisoned_set_loader = torch.utils.data.DataLoader(
        poisoned_set,
        batch_size=batch_size, shuffle=True, worker_init_fn=tools.worker_init, **kwargs)

elif 'freq' in args.dataset or args.dataset == 'pattern' or args.dataset == 'wanet':
    target_class = 0 if 'freq' in args.dataset else 2
    poisoned_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/poisonTrain_pr_{}_t_{}'.format(args.dataset, args.poison_rate, target_class)
    poison_indices_path = os.path.join(poisoned_set_img_dir, 'poison_file.npy')

    print('dataset : %s' % poisoned_set_img_dir)
    poisoned_set = tools.IMAGE_Dataset(data_dir=poisoned_set_img_dir, transforms=data_transform if args.no_aug else data_transform_aug)
...
if args.dataset != 'ember' and args.dataset != 'imagenet' and args.dataset != 'ultrasonic' and args.dataset != 'pattern' and args.dataset != 'freq' and args.dataset != 'freq_meg_500' and args.dataset != 'wanet':
    ...
    test_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/Test'
    test_set = tools.AUDIO_Dataset(data_dir=test_set_img_dir, transforms=None)
    test_set_loader = torch.utils.data.DataLoader(
        test_set,
        batch_size=batch_size, shuffle=False, worker_init_fn=tools.worker_init, **kwargs)
    poison_test_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/poisonTest'
    classes = ['down', 'go', 'left', 'no', 'off', 'on', 'right', 'stop', 'up', 'yes']
    label = classes.index('down')
    poison_test_set = tools.AUDIO_Dataset(data_dir=poison_test_set_img_dir, transforms=None, fixed_label=label)
    poison_test_set_loader = torch.utils.data.DataLoader(
        poison_test_set,
        batch_size=batch_size, shuffle=False, worker_init_fn=tools.worker_init, **kwargs)

elif 'freq' in args.dataset or args.dataset == 'pattern' or args.dataset == 'wanet':
    test_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/Test'.format(args.dataset)
    test_set = tools.IMAGE_Dataset(data_dir=test_set_img_dir, transforms=data_transform)
    test_set_loader = torch.utils.data.DataLoader(
        test_set,
        batch_size=batch_size, shuffle=False, worker_init_fn=tools.worker_init, **kwargs)
    poison_test_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/poisonTest'.format(args.dataset)
    classes = [f for f in os.listdir(poison_test_set_img_dir) if not f.endswith('.npy')]
    target_class_name = 'airplane' if args.dataset == 'freq' else 'bird'
    label = classes.index(target_class_name)
    poison_test_set = tools.IMAGE_Dataset(data_dir=poison_test_set_img_dir, transforms=data_transform, fixed_label=label)
    poison_test_set_loader = torch.utils.data.DataLoader(
        poison_test_set,
        batch_size=batch_size, shuffle=False, worker_init_fn=tools.worker_init, **kwargs)
    ...
    if args.dataset != 'ember':
        ...
        # if epoch % 5 == 0:
            if args.dataset == 'imagenet':
                ...
                tools.test(model=model, test_loader=test_set_loader, poison_test=False, num_classes=num_classes, source_classes=source_classes,
                           all_to_all=all_to_all, args=args)
                print('ASR testing')
                tools.test(model=model, test_loader=poison_test_set_loader, poison_test=False   , num_classes=num_classes,
                           source_classes=source_classes, all_to_all=all_to_all, args=args)
                torch.save(model.state_dict(), supervisor.get_model_dir(args))
            elif 'freq' in args.dataset or args.dataset == 'pattern' or args.dataset == 'wanet':
                tools.test(model=model, test_loader=test_set_loader, poison_test=False, num_classes=num_classes, source_classes=source_classes,
                           all_to_all=all_to_all, args=args)
                ...
                tools.test(model=model, test_loader=test_set_loader, poison_test=True,
                           poison_transform=poison_transform, num_classes=num_classes, source_classes=source_classes, all_to_all=all_to_all, args=args)
                torch.save(model.state_dict(), supervisor.get_model_dir(args))
        ...
        tools.test_ember(model=model, test_loader=test_set_loader,
                             backdoor_test_loader=backdoor_test_set_loader, args=args)
