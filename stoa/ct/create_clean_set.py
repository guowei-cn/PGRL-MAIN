...
parser.add_argument('-clean_budget', type=int, default=10)
...
elif args.dataset in ['imagenette_pattern', 'imagenette_adaptivecifar10', 'imagenette_freq_meg_500']:
    data_transform = transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.ToTensor()
    ])
    if os.path.exists(os.path.join(data_dir, args.dataset)) is False:
        clean_set = datasets.Imagenette(os.path.join(data_dir, args.dataset), split='train',
                                        download=True, transform=data_transform)
    else:
        clean_set = datasets.Imagenette(os.path.join(data_dir, args.dataset), split='train',
...
if args.dataset != 'ember' and args.dataset != 'imagenet':
    ...
    if args.dataset == 'imagenette':
        ...
        clean_split_indices, test_indices = [], []
        for class_i in range(len(clean_set.classes)):
            cnt = 0
            for id in id_set:
                if clean_set.targets[id] == class_i:
                    clean_split_indices.append(id)
                    cnt += 1
                if cnt == int(args.clean_budget/len(clean_set.classes)):
                    break

        for id in id_set:
            if id not in clean_split_indices:
