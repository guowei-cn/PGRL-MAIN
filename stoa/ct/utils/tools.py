...
class AUDIO_Dataset(Dataset):
    def __init__(self, data_dir, transforms = None, num_classes = 10, shift = False, random_labels = False,
                 fixed_label = None):
        """
        Args:
            data_dir: directory of the data
    ...
    def __getitem__(self, idx):
        ...
        img = torch.matmul(melspectrum.transpose(-1, -2), self.dct_mat)
...
def test(model, test_loader, poison_test = False, poison_transform=None, num_classes=10, source_classes=None, all_to_all = False, args=None):
...
def test_imagenet(model, test_loader, test_backdoor_loader=None, args=None):
...
def test_ember(model, test_loader, backdoor_test_loader, args=None):
    model.eval()
    clean_correct = 0
    tot = 0
    with torch.no_grad():
