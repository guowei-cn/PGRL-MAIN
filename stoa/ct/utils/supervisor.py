...
import argparse
import stoa.ct.config as config
from stoa.ct.poison_tool_box import badnet, trigger_10, trojan, blend, clean_label, WaNet, adaptive_blend, \
    adaptive_patch, SIG, TaCT, none
...
def get_poison_set_dir(args):
    ...
    if args.poison_type == 'blend' or args.poison_type == 'basic' or args.poison_type == 'clean_label':
        blend_alpha = '%.3f' % args.alpha
        poison_set_dir = '/storageA/david_projects/PGRL-main/stoa/ct/poisoned_train_set/%s/%s_%s_alpha=%s_trigger=%s' % (args.dataset, args.poison_type, ratio, blend_alpha, args.trigger)
    elif args.poison_type == 'adaptive' or args.poison_type == 'adaptive_blend':
        blend_alpha = '%.3f' % args.alpha
        cover_rate = '%.3f' % args.cover_rate
        poison_set_dir = '/storageA/david_projects/PGRL-main/stoa/ct/poisoned_train_set/%s/%s_%s_alpha=%s_cover=%s_trigger=%s' % (args.dataset, args.poison_type, ratio, blend_alpha, cover_rate, args.trigger)
    elif args.poison_type == 'adaptive_patch' or args.poison_type == 'TaCT' or args.poison_type == 'WaNet':
        cover_rate = '%.3f' % args.cover_rate
        poison_set_dir = '/storageA/david_projects/PGRL-main/stoa/ct/poisoned_train_set/%s/%s_%s_cover=%s' % (args.dataset, args.poison_type, ratio, cover_rate)
    elif args.poison_type == 'adaptive_patch' or args.poison_type == 'adaptive_physical':
        poison_set_dir = '/storageA/david_projects/PGRL-main/stoa/ct/poisoned_train_set/%s/%s_%s_trigger=%s' % (args.dataset, args.poison_type, ratio, args.trigger)
    else:
        poison_set_dir = '/storageA/david_projects/PGRL-main/stoa/ct/poisoned_train_set/%s/%s_%s' % (args.dataset, args.poison_type, ratio)
...
def get_poison_transform(poison_type, dataset_name, target_class, source_class=1, cover_classes=[5,7],
    ...
    if poison_type in ['badnet', 'blend', 'clean_label',
        ...
        if trigger_name != 'none': # none for SIG
            ...
            trigger = trigger_transform(trigger).to(args.device)
            # print('trigger_shape: ', trigger.shape)
            trigger_mask = trigger_mask.to(args.device)
        ...
        elif poison_type == 'trigger_10':
            poison_transform = trigger_10.poison_transform(img_size=img_size, trigger_mark=trigger, trigger_mask=trigger_mask, target_class=target_class)
        ...
        poison_transform = poison_tool_box.dynamic.poison_transform(ckpt_path=ckpt_path, channel_init=channel_init, steps=steps,
        ...
        poison_transform = poison_tool_box.ISSBA.poison_transform(ckpt_path=ckpt_path, secret=secret, normalizer=normalizer, denormalizer=denormalizer,
