...
arch = {
    ...
    'freq': resnet.ResNet18,
    'freq_meg_500': resnet.ResNet18,
    'pattern': resnet.ResNet18,
    'wanet': resnet.ResNet18,
    'ultrasonic': resnet.CNN,
...
def get_dataset(inspection_set_dir, data_transform, args, num_classes = 10):
    ...
    if args.dataset == 'ultrasonic':
        ...
        inspection_set = tools.AUDIO_Dataset(data_dir=poisoned_set_img_dir, transforms=True)

        clean_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/valTrain'.format(
            args.poison_rate)

        print('clean dataset : %s' % clean_set_img_dir)

        clean_set = tools.AUDIO_Dataset(data_dir=clean_set_img_dir, transforms=True)
    elif 'freq' in args.dataset or args.dataset == 'pattern' or args.dataset == 'wanet':
        target_class = 0 if 'freq' in args.dataset else 2
        poisoned_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/poisonTrain_pr_{}_t_{}'.format(
            args.dataset, args.poison_rate, target_class)

        print('inspected dataset : %s' % poisoned_set_img_dir)

        inspection_set = tools.IMAGE_Dataset(data_dir=poisoned_set_img_dir, transforms=data_transform)

        clean_set_img_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/valTrain'.format(args.dataset)

        print('clean dataset : %s' % clean_set_img_dir)

        clean_set = tools.IMAGE_Dataset(data_dir=clean_set_img_dir, transforms=data_transform)
    else:
...
def get_packet_for_debug(poison_set_dir, data_transform, batch_size, args):
    if args.dataset == 'ultrasonic':
        ...
        test_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/Test'
        test_set = tools.AUDIO_Dataset(data_dir=test_set_dir)
        ...
        poison_test_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/poisonTest'
        poison_test_set = tools.AUDIO_Dataset(data_dir=poison_test_set_dir)

        kwargs = {'num_workers': 2, 'pin_memory': True}
        poison_test_set_loader = torch.utils.data.DataLoader(
            poison_test_set,
            batch_size=256, shuffle=True, worker_init_fn=tools.worker_init, **kwargs)
        poison_train_folder = '/storageA/david_projects/DefTimeSeries/poisonDataset/ultrasonic/poisonTrain_pr_{}_t_down'.format(args.poison_rate)
        poison_ds = tools.AUDIO_Dataset(data_dir=poison_train_folder)
        poison_files = np.load(os.path.join(poison_train_folder, 'poison_file.npy'))
        poison_indices = []
        for index, f in enumerate(poison_ds.files):
            f = f.split('ultrasonic')[1][1:]
            if f in poison_files:
                poison_indices.append(index)
        ...
        debug_packet = {
            ...
            'poison_test_set_loader': poison_test_set_loader,
            'poison_indices': poison_indices,
            'source_classes': source_classes
        }
    elif 'freq' in args.dataset or args.dataset == 'pattern' or args.dataset == 'wanet':
        # Set Up Test Set for Debug & Evaluation
        test_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/Test'.format(args.dataset)
        test_set = tools.IMAGE_Dataset(data_dir=test_set_dir, transforms=data_transform)
        ...
        poison_test_set_dir = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/poisonTest'.format(args.dataset)
        poison_test_set = tools.IMAGE_Dataset(data_dir=poison_test_set_dir, transforms=data_transform)
        ...
        poison_test_set_loader = torch.utils.data.DataLoader(
            ...
            batch_size=256, shuffle=True, worker_init_fn=tools.worker_init, **kwargs)
        target_class = 0 if 'freq' in args.dataset else 2
        poison_train_folder = '/storageA/david_projects/DefTimeSeries/poisonDataset/{}/poisonTrain_pr_{}_t_{}'.\
            format(args.dataset, args.poison_rate, target_class)
        poison_ds = tools.IMAGE_Dataset(data_dir=poison_train_folder, transforms=data_transform)
        poison_files = np.load(os.path.join(poison_train_folder, 'poison_file.npy'))
        poison_indices = []
        for index, f in enumerate(poison_ds.data):
            f = f.split(args.dataset)[1][1:]
            if f in poison_files:
                poison_indices.append(index)
        ...
        poison_transform = supervisor.get_poison_transform(poison_type=args.poison_type, dataset_name=args.dataset,
                                                           target_class=target_class_origin[args.dataset],
