...
def visualize(losses, poisoned_vector, path):
    ...
    plt.xlabel('entropy')
    plt.ylabel('PDF')
    plt.savefig('pictures/' + path)
    plt.close(fig)

def eval_train(args, model, pre_loss, eval_loader1, poisoned_vector, epoch, threshold):
    print('\n')
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (inputs1, targets1, index) in enumerate(eval_loader1):
            ...
            for b in range(inputs1.size(0)):
                losses[index[b]] = -torch.mean(torch.mul(logits_softmax1[b, :], torch.log(logits_softmax1[b, :]+1e-5)))
    ...
    return np.ones_like(losses), pred1, pre_loss


def warmup(args, epoch, net, optimizer, scalar, dataloader):
    net.train()
...
def train_poisoned_model(args, epoch, net, optimizer, scalar, labeled_trainloader):
    net.train()
    ...
    for batch_idx, (inputs_x, _, _, labels_x, w_x) in enumerate(labeled_trainloader):
        ...
        with autocast():
            logits = net(inputs_x)
            loss = -torch.mean(torch.sum(F.log_softmax(logits, dim=1) * labels_x, dim=1))
...
def save_state(epoch, net, optimizer, scalar, poisoned_index, noisy_idx, pre_loss, threshold, path):
    saved_dict = {
        "epoch": epoch,
...
def load_state(net, optimizer, path):
    ...
    return epoch, scalar, poisoned_index, noisy_idx, pre_loss, threshold