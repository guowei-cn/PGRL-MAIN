...
def attention_cutmix(inputs, attentions, targets, threshold):
    ...
    mixed_target = targets * weight + targets_shuffle * (1 - weight)
...
def train_attention_mix(args, epoch, net, optimizer, scalar, labeled_trainloader):
    net.train()
...
def train_attention_mix_mixmatch(args, epoch, net, net2, optimizer, scalar, labeled_trainloader, unlabeled_trainloader, prob2, pred2, loader):
    net.train()
    net2.eval()
    ...
    for batch_idx in range(0, num_iter):
        ...
        with autocast():
            ...
            targets_mix = mixed_labels[:batch_size * 2]
        ...
        sys.stdout.write(
            ...
            % (args.dataset, epoch, args.num_epochs, batch_idx + 1, num_iter,
               Lmix.item(), Lu.item(), mixed_region_sum / (batch_idx + 1), correct, sum, wrong, sum1, correct_u, sum_u, wrong_u, sum1_u))
        sys.stdout.flush()
    print('\nDelete: ' + str(count))
