...
def train_attention_mix(args, epoch, net, optimizer, labeled_trainloader, device):
    net.train()
    ...
    for batch_idx, (inputs_x, inputs_x2, _, labels_x, poisoned) in enumerate(labeled_trainloader):
        ...
        inputs_x, inputs_x2, labels_x, poisoned = inputs_x.to(device), inputs_x2.to(device), labels_x.to(device), poisoned.to(device)
...
def train_attention_mix_mixmatch(args, epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, prob2, pred2, loader, device):
    net.train()
    net2.eval()
    ...
    for batch_idx in range(0, num_iter):
        try:
            inputs_x, inputs_x2, inputs_x3, labels_x, poisoned = next(labeled_train_iter)
        except:
            labeled_train_iter = iter(labeled_trainloader)
            inputs_x, inputs_x2, inputs_x3, labels_x, poisoned = next(labeled_train_iter)
        try:
            inputs_u, inputs_u2, inputs_u3, labels_u, poisoned_u = next(unlabeled_train_iter)
        except:
            unlabeled_train_iter = iter(unlabeled_trainloader)
            inputs_u, inputs_u2, inputs_u3, labels_u, poisoned_u = next(unlabeled_train_iter)
        ...
        inputs_x, inputs_x2, inputs_x3, labels_x, poisoned = inputs_x.to(device), inputs_x2.to(device), inputs_x3.to(device), labels_x.to(device), poisoned.to(device)
        inputs_u, inputs_u2, inputs_u3, labels_u, poisoned_u = inputs_u.to(device), inputs_u2.to(device), inputs_u3.to(device), labels_u.to(device), poisoned_u.to(device)

        with torch.no_grad():
            outputs_u11, feature_maps_u = net(inputs_u)
            B_net_asr += torch.sum((torch.argmax(outputs_u11, dim=1) == args.trigger_label).float() * poisoned_u.squeeze()).item()
            asr_count += torch.sum(poisoned_u).item()
            outputs_u12, _ = net(inputs_u2)
            outputs_u_net2 = net2(inputs_u3)
            V_net_asr += torch.sum((torch.argmax(outputs_u_net2, dim=1) == args.trigger_label).float() * poisoned_u.squeeze()).item()
        ...
        sys.stdout.flush()
    print('B_net_asr: ' + str(B_net_asr/asr_count))
    print('V_net_asr: ' + str(V_net_asr/asr_count))
    print('\nDelete: ' + str(count))
