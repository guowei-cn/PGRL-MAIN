...
parser.add_argument('--posioned_portion', default=0.05, type=float)
parser.add_argument('--cover_portion', default=0.0, type=float)
...
loader = cifar10_dataloader(args, args.batch_size, 4, args.data_path, args.trigger_label, args.poisoned_mode, args.posioned_portion, args.cover_portion, poisoned_idx, noisy_idx)
...
poisoned_vector = np.zeros(len(loader.train_data))
for i in poisoned_idx:
    poisoned_vector[i] = 1

ACC = test(0, net1, net2, test_CL_loader1, test_CL_loader2, test_log, device)
print('ACC B-net1 {:.2f}% and V-net2 {:.2f}%'.format(ACC[0], ACC[1]))
ASR = test(0, net1, net2, test_BD_loader1, test_BD_loader2, test_log, device)
print('ASR B-net1 {:.2f}% and V-net2 {:.2f}%'.format(ASR[0], ASR[1]))


if start_epoch2 == 0:
    print('\nWarmup Net2')
    for _ in range(2):
        warmup(args, 0, net2, optimizer2, warmup_trainloader, device)
    pre_loss = []

for epoch in range(start_epoch2+1, args.warm_up):
    print('\nWarmup Net2')
    threshold = max(1-0.2*epoch, 0.2)
    prob2, pred2, threshold, pre_loss = eval_train(args, net2, pre_loss, eval_loader2, poisoned_vector, epoch-1, threshold, device)

    labeled_suspicious_loader, _ = loader.run('train_net2', 1 - pred2, 1 - prob2)
    train_poisoned_model(args, epoch, net2, optimizer2, labeled_suspicious_loader, device)
    ACC = test(epoch, net1, net2, test_CL_loader1, test_CL_loader2, test_log, device)
    print('ACC B-net1 {:.2f}% and V-net2 {:.2f}%'.format(ACC[0], ACC[1]))
    ASR = test(epoch, net1, net2, test_BD_loader1, test_BD_loader2, test_log, device)
    print('ASR B-net1 {:.2f}% and V-net2 {:.2f}%'.format(ASR[0], ASR[1]))

prob2, pred2, threshold, pre_loss = eval_train(args, net2, pre_loss, eval_loader2, poisoned_vector, args.warm_up-1, threshold, device)
...
for epoch in range(start_epoch1+1, args.num_epochs+1):
    print('\nTrain Net1')
    if epoch <= 100:
        train_attention_mix(args, epoch, net1, optimizer1, labeled_credible_trainloader, device)  # train net1
    else:
        train_attention_mix_mixmatch(args, epoch, net1, net2, optimizer1, labeled_credible_trainloader, unlabeled_suspicious_trainloader, prob2, pred2, loader, device)
    scheduler1.step()
    ...
    if epoch % 5 == 0 or epoch > 100:
        ...
        prob2, pred2, threshold, pre_loss = eval_train(args, net2, pre_loss, eval_loader2, poisoned_vector, epoch+args.warm_up-1, threshold, device)
        labeled_credible_trainloader, unlabeled_suspicious_trainloader = loader.run('train_net1', pred2, prob2, args.batch_size)
