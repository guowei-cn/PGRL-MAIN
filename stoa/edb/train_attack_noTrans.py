...
def train_epoch(arg, trainloader, model, optimizer, scheduler, criterion, epoch):
    ...
    train_loss = 0
    avg_acc_clean, avg_acc_attack, avg_acc_robust = -1, -1, -1
    for i, (inputs, labels, gt_labels, isCleans) in enumerate(trainloader):
        ...
        total_robust_correct += torch.sum(torch.argmax(outputs[clean_idx], dim=1) == labels[clean_idx])
        ...
        #              'Epoch: %d | Loss: %.3f | Total ACC: %.3f%% (%d/%d) | Train ASR: %.3f%% (%d/%d) | Train ACC: %.3f%% (%d/%d)' % (
    ...
    scheduler.step()
    print('Total ACC: {:.3f}, ASR: {:.3f}, ACC: {:.3f}'.format(avg_acc_clean, avg_acc_attack, avg_acc_robust))
...
def test_epoch(arg, testloader, model, criterion, epoch, word):
    ...
    if word == 'clean':
        print('acc: {:.3f}'.format(avg_acc_clean))
    if word == 'bd':
        print('asr: {:.3f}'.format(avg_acc_clean))

    return test_loss / (i + 1), avg_acc_clean, -1
...
def main():
    ...
    # Dataset #TODO: replace with my dataset
    ...
    writer.writerow(
        ['Epoch', 'Train_Loss', 'Train_ACC', 'Train_ASR', 'Test_Loss_cl', 'Test_ACC', 'Test_Loss_bd', 'Test_ASR'])
    ...
    for epoch in tqdm(range(start_epoch, arg.epochs)):
        ...
        test_loss_bd, test_acc_bd, _ = test_epoch(arg, testloader_bd, model, criterion, epoch, 'bd')
        ...
        writer.writerow(
            [epoch, train_loss, train_acc.item(), train_asr.item(), test_loss_cl, test_acc_cl.item(),
             test_loss_bd, test_acc_bd.item()])
    csvFile.close()
