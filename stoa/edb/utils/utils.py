...
def normalization(opt, inputs):
    output = inputs.clone()
    if opt.dataset == "cifar10" or opt.dataset =='adaptivecifar10' or opt.dataset == 'blto' or 'freq' in opt.dataset or 'pattern' in opt.dataset or 'wanet' in opt.dataset or 'imagenette' in opt.dataset:
        f = transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])
    elif opt.dataset == "ultrasonic":
        f = transforms.Normalize([0.], [1])
