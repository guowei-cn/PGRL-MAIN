...
ablation_dict = {'all': (1., 1.), 'lcv': (1., 0.), 'fde': (0., 1.)}
...
def update_weights(old_weights, old_weights_indices, untrusted_act_l, untrusted_label_l, indices_l, gt_poison_l, weight_act_l, val_weight_est_lables,
    ...
    sorted_indices = np.argsort(indices_l)
    indices_l_sorted = indices_l[sorted_indices]
    gt_poison_l_sorted = gt_poison_l[sorted_indices]
    ...
    for i in np.unique(untrusted_label_l):
        ...
        umap_model_10d = umap.UMAP(n_components=10, n_jobs=-1, metric='cosine') #, random_state=42)
        ...
        umap_model_2d = umap.UMAP(n_components=2, n_jobs=-1, metric='cosine') #, random_state=42)
    ...
    # momuntom
    auc_score = roc_auc_score(gt_poison_l_sorted, -sorted_weight_l)
...
def ssl_training_weight(epoch_id, weights, weights_indices, poison_type, poison_ratio, model, ssl_dl, val_dl, criterion,
    ...
    flag_lcv, flag_fde = ablation_dict[xloss]
    for b_id, batch in tqdm(enumerate(ssl_dl), total=len(ssl_dl)):
        ...
        if batch_id < warmup_epoch * len(ssl_dl): # warm-up
            ...
            loss_ce = F.cross_entropy(outputs, l_labels.to(device), reduction='mean')
        else:
            ...
            loss_ce = torch.mean(torch.cat([loss_ce, losses_high * weights_query_high]))
        ...
        if epoch_id % vis_repeat == 0:
            ...
            if flag_fde:
                weights, weights_indices = update_weights(weights, weights_indices, untrusted_act_l, untrusted_label_l,
                                                          ...
                                                          'picture/{}_{}_{}_hist_{}_weights_{}_emb_as_act.png'.format(poison_type, poison_ratio, num_sample, epoch_id, model_name), threshold_percent)
                ...
                weights, weights_indices = weights, weights_indices
...
def get_hyperparameters(poison_type, model_name='cnn'):
    if poison_type == 'adaptivecifar10' or 'freq_meg_500' == poison_type or poison_type == 'pattern' \
        ...
        lr, final_lr, epoch_num = 0.001, 0.0001, 100
    else:
...
def main(args):
    ...
    # if 'imagenette' in poison_type and args.xloss == 'fde':
    #     optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.99)
    # else:
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=0)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [80], 0.1)
    ...
    for epoch in range(1, epoch_num+1):
        ...
        batch_id, weights, weights_indices = ssl_training_weight(epoch, weights, weights_indices, poison_type, poison_rate,
                                     ...
                                     warm_up_epoch, xloss, num_sample, threshold_percent, model_name,
                                     writer=writer, vis_repeat=vis_repeat)
        if scheduler != None:
            scheduler.step()
        if epoch % 5 == 0:
...
if __name__ == '__main__':
    ...
    def parse_args():
        ...
        parser.add_argument('-d', '--device', default='cuda:1', type=str, help='The device to use (e.g., "cpu" or "cuda").')
